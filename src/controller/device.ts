...
async function ensureDeviceTable(fastify: FastifyInstance) {
    ...
    try {
        ...
        await pgClient.query(`ALTER TABLE devices ADD COLUMN IF NOT EXISTS browser TEXT`);
        await pgClient.query(`ALTER TABLE devices ADD COLUMN IF NOT EXISTS os_version TEXT`);
        await pgClient.query(`ALTER TABLE devices ADD COLUMN IF NOT EXISTS app_version TEXT`);
        ...
        await pgClient.query(`ALTER TABLE devices ADD COLUMN IF NOT EXISTS attestation_passed BOOLEAN NOT NULL DEFAULT TRUE`);
        await pgClient.query(`ALTER TABLE devices ADD COLUMN IF NOT EXISTS last_attestation_at TIMESTAMPTZ`);
        await pgClient.query(`ALTER TABLE devices ADD COLUMN IF NOT EXISTS attestation_token TEXT`);
        await pgClient.query(`ALTER TABLE devices ADD COLUMN IF NOT EXISTS is_emulator BOOLEAN NOT NULL DEFAULT FALSE`);
        await pgClient.query(`ALTER TABLE devices ADD COLUMN IF NOT EXISTS is_rooted_jailbroken BOOLEAN NOT NULL DEFAULT FALSE`);
...
async function deviceController(fastify: FastifyInstance) {
    ...
    fastify.get(`${uri}/`, {
        preHandler: [(fastify as any).authorize([USER_ROLE_TYPES.ADMIN, USER_ROLE_TYPES.INSTRUCTOR])],
        schema: {
            querystring: {
                type: 'object',
                properties: {
        ...
        try {
            ...
            const result = await pgClient.query(
                `SELECT d.id, d.user_id, u.email, u.full_name, d.device_fingerprint,
                        d.device_name, d.platform, d.browser, d.os_version, d.app_version,
                        d.attestation_passed, d.is_emulator, d.is_rooted_jailbroken,
                        d.is_trusted, d.trust_score,
                        d.is_active, 
    ...
    fastify.post(`${uri}/register`, {
        preHandler: [(fastify as any).authorize(1)],
        schema: {
            body: {
                type: 'object',
                ...
                properties: {
                    ...
                    deviceDetectedEmulator: { type: 'boolean' },
                    deviceDetectedRooted: { type: 'boolean' }
                },
                additionalProperties: false
            }
        }
        ...
        const attestation = await deviceAttestationService({
            platform,
            platformAttestation,
            deviceDetectedEmulator,
            deviceDetectedRooted
        ...
        try {
            const result = await pgClient.query(
                `INSERT INTO devices (
                    id, user_id, device_fingerprint, device_name, platform, browser, os_version, app_version,
                    public_key, public_key_created_at,
                    attestation_passed, last_attestation_at, attestation_token, is_emulator, is_rooted_jailbroken,
                ...
                DO UPDATE SET
                    ...
                    attestation_token = COALESCE(EXCLUDED.attestation_token, devices.attestation_token),
                ...
                RETURNING id, device_fingerprint, device_name, platform,
                          browser, os_version, app_version,
                ...
                [
                    ...
                    attestation.isEmulator,
                    attestation.isRootedJailbroken
                ]
            );
    ...
    fastify.get(`${uri}/my-devices`, {
        ...
        try {
            const result = await pgClient.query(
                `SELECT id, device_name, platform, browser, os_version, app_version,
                        attestation_passed, is_emulator, is_rooted_jailbroken,
