...
async function checkinController(fastify: FastifyInstance) {
    ...
    fastify.post(uri, {
        ...
        try {
            ...
            // liveness_challenge_response is a base64 token generated by the
            // frontend liveness challenge UI: btoa('liveness_<timestamp>_<random>').
            // Verifying the format confirms the student completed the challenge.
            ...
            if (liveness_challenge_response && typeof liveness_challenge_response === 'string') {
                try {
                    ...
                    // Expect format: liveness_<timestamp_ms>_<random>
                    if (parts.length >= 3 && parts[0] === 'liveness') {
                        const ts = Number(parts[1]);
                        const tokenAge = Date.now() - ts;
                        // Token must be fresh (< 10 minutes old) and have non-empty random
                        if (Number.isFinite(ts) && tokenAge >= 0 && tokenAge < 600_000 && (parts[2]?.length ?? 0) > 0) {
                            livenessPassed = true;
                            livenessScore = 0.75; // Standard passing score without ML
                        }
                    }
                } catch {
            ...
            // Weights: geolocation 15%, device attestation 20%, liveness 25%,
            //          face match 25% (0 without ML), network 15% (0 without detection)
            ...
            // Face match and network (no ML/detection yet — zero contribution)
